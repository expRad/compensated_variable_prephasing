...
diff_phase = angle(kspace); % dimensions: [numROP, coils, numPE, numPE, slices, triangles, 1, 1, measurements(if>1)]
diff_phase(2:end,:,:,:,:,:,:,:,:) = diff(diff_phase,1,1);
% Add a zero as the first phase derivative value, so the array size stays the same.
diff_phase(1,:,:,:,:,:,:,:,:) = 0;
diff_phase = (mod(diff_phase+pi, 2*pi) - pi);
diff_phase = diff_phase/dwelltime;
...
if singleCoil==0
    ...
    if weigh_equal
        ...
        insert_weight = weights_squared(10,:,:,:,:,3);
        for rop=1:size(weights_squared,1)
            for meas=1:size(weights_squared,6)
                weights_squared(rop,:,:,:,:,meas) = insert_weight;
            end
        end
