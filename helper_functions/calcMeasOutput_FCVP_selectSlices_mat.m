function [ out_grad, dwelltime, out_VPonly, invcovmat ] = calcMeasOutput_FCVP_selectSlices_mat( file, numRepPerGrad, whichGrad, numGrad, calcChannels, singleCoil, skipCoils, nDummies, compConcomit, deleteSlices)
% This function calculates the measured gradient output for the 
% "compensated" and the "fully compensated" variable-prephasing methods.
...
numMeas = size(kspace,6); % number of measurements = numGrad*numVP*2
...
if compConcomit
    ...
    coeff_mat = zeros(size(diff_phase,1), (1+numVP)*N_L+2*N_p); % [4*numVP*numPositions, (1+numVP)*numChannels+2*numPositions]
    coeff_mat_VPonly = zeros(size(diff_phase,1)/2, numVP*N_L+N_p); % [2*numVP*numPositions, numVP*numChannels+numPositions]
end
...
if compConcomit
    ...
    coeff_mat(numVP*N_p+1:2*numVP*N_p,1:N_L) = 0;
end

tmp = zeros(numVP*N_p, numVP*N_L);
disp(['size(tmp) = ',num2str(size(tmp))]);
for i=0:numVP-1
...
if compConcomit
    ...
    coeff_mat(:,N_L+1:(1+numVP)*N_L) = cat(1,tmp,tmp*0,tmp,tmp*0);
    coeff_mat_VPonly(:,1:numVP*N_L) = cat(1,tmp,tmp*0);
end
...
if compConcomit
    ...
    coeff_mat(:,(1+numVP)*N_L+1:(1+numVP)*N_L+N_p) = cat(1,repmat(eye(N_p),[2*numVP 1]),zeros(numVP*N_p*2,N_p));
    coeff_mat(:,(1+numVP)*N_L+N_p+1:end) = cat(1,zeros(numVP*N_p*2,N_p),repmat(eye(N_p),[2*numVP 1]));
    coeff_mat_VPonly(:,numVP*N_L+1:end) = repmat(eye(N_p),[2*numVP 1]);
end
...
if ~compConcomit
    sol_mat_VPonly = zeros(size(coeff_mat,2)-calcChannels-numPositions, size(diff_phase,2)); % [numVP*numChannels+numPositions, numTimePoints]
    for t = 1:size(diff_phase,2)
        f = diff_phase(end/2+1:end,t); % [2*numVP*numPositions, 1]
        m = magnitude(end/2+1:end,t); % [2*numVP*numPositions, 1]
        invcov = zeros(size(f,1));
        for i = 1:size(f,1)
            invcov(i,i) = m(i)*m(i);
        end
        invcov(end,end) = m(end)*m(end);
        y = (transpose(coeff_mat_VPonly)*invcov*coeff_mat_VPonly) \ (transpose(coeff_mat_VPonly)*invcov*f);
        sol_mat_VPonly(:,t) = y;
    end
end
...
if ~compConcomit
    out_VPonly = 1/gamma * sol_mat_VPonly;
else
    out_VPonly = [];
end
