...
matrix_size = [340, 136];
nSlices = 2;
recos_del = zeros(matrix_size(1), matrix_size(2), nSlices, nMeas, length(delays_nom));
recos_girf = zeros(matrix_size(1), matrix_size(2), nSlices, nMeas, length(delays_girf));
recos_meas = zeros(matrix_size(1), matrix_size(2), nSlices, nMeas, length(delays_meas));
sum_out_del = zeros(size(squeeze(recos_del(1,1,:,:,:))));
sum_out_girf = zeros(size(squeeze(recos_del(1,1,:,:,:))));
sum_out_meas = zeros(size(squeeze(recos_del(1,1,:,:,:))));
...
%% Read data for measured trajectory
% Per default, the CVP-solution is used here:
[ grad_traj_orig, dwelltime_traj ] = calcMeasOutput_FCVP_selectSlices_mat(name2load_traj, 1, 1, 1, 2, 0, [], 10, 0, []);
% Use the following line for the VP-solution:
% [ grad_traj_orig, dwelltime_traj ] = calcMeasOutput_VP_selectSlices_mat(name2load_traj, 1, 1, 1, 2, 0, [], 10, 0, []);
% Use the following line for the FCVP-solution (use the correct measurement for this!!)
% [ grad_traj_orig, dwelltime_traj ] = calcMeasOutput_FCVP_selectSlices_mat(name2load_traj, 1, 1, 1, 2, 0, [], 10, 1, []);

%%
for loopcount = 1:length(delays_nom)
    nom_delay = delays_nom(loopcount);
    girf_delay = delays_girf(loopcount);
    meas_delay = delays_meas(loopcount);
    ...
    %% CALCULATE NOMINAL AND DELAY-CORRECTED K-SPACE TRAJECTORY
    ...
    % GSTF-based trajectory prediction
    ...
    max_kRO_meas = max(abs(kRO_meas),[],'all');
    max_kRO = max(max_kRO_nom, max(max_kRO_girf, max_kRO_meas));
    ...
    traj_meas = (kRO_meas + 1i*kPE_nom)/max_k;
    traj_factor = 1.9;
    ...
    raw_meas_filled = cat(3, zeros(size(raw(:,:,1:nPad,:,:))), raw_meas);
    ...
    traj_meas_filled = cat(2,traj_fill,traj_meas);
    ...
    FT_meas_dc = cGrid_Cartesian(traj_meas_filled/traj_factor,matrix_size,dc_filled);
    %%
    reco_nom_dc = zeros(nRO, nImageLines, nCoils, nSlices, nMeas);
    reco_girf_dc = zeros(size(reco_nom_dc));
    reco_meas_dc = zeros(size(reco_nom_dc));
    ...
    for coil = 1:nCoils
        for slice = 1:nSlices
            for meas = 1:nMeas*nAvg
                ...
                reco_meas_dc(:,:,coil,slice,meas) = FT_meas_dc'*squeeze(raw_meas_filled(:,coil,:,slice,meas));
            end
        end
    end
    ...
    if nCoils == 1
        ...
        sum_images_meas_dc = squeeze(reco_meas_dc(:,:,1,:,:));
    elseif SoS
        ...
        sum_images_meas_dc = squeeze(sqrt(sum(reco_meas_dc.*conj(reco_meas_dc), 3)));
    else
        % only take single coil image
        sum_images_nom_dc = squeeze(reco_nom_dc(:,:,coil_i,:,:));
        sum_images_girf_dc = squeeze(reco_girf_dc(:,:,coil_i,:,:));
        sum_images_meas_dc = squeeze(reco_meas_dc(:,:,coil_i,:,:));
    end
    
    %%
    recos_del(:,:,:,:,loopcount) = sum_images_nom_dc;
    recos_girf(:,:,:,:,loopcount) = sum_images_girf_dc;
    recos_meas(:,:,:,:,loopcount) = sum_images_meas_dc;

    [xx,yy] = meshgrid((1:size(recos_meas,2)),(1:size(recos_meas,1)));
    mask_out = double(sqrt((xx-x0).^2+(yy-y0).^2)>r_out);

    for slice=1:nSlices
        for meas=1:nMeas
            sum_out_del(slice,meas,loopcount) = sum(squeeze(sum_images_nom_dc(:,:,slice,meas)).*mask_out,'all');
            sum_out_girf(slice,meas,loopcount) = sum(squeeze(sum_images_girf_dc(:,:,slice,meas)).*mask_out,'all');
            sum_out_meas(slice,meas,loopcount) = sum(squeeze(sum_images_meas_dc(:,:,slice,meas)).*mask_out,'all');
        end
    end
...
%% SAVE RESULTS
save([path2save,name2save],'sum_out_del','sum_out_girf','sum_out_meas','delays_nom','delays_girf','delays_meas','B0_correction','x0','y0','r_out');
disp('Saved results.');
